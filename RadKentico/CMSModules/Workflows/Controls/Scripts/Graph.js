    ...
    */
    createNodes: function (definitions) {
        for (var i = 0; i < definitions.length; i++) {
            var nodeJQ = $j("#" + this.graphJQ.attr('id') + "_" + definitions[i].ID);
            if (nodeJQ.length > 0) {
    ...
    */
    setEditingHandlers: function () {
        this.containerJQ.droppable({
            ...
            activate: function () { $j(".UniGraph").addClass("Move") },
            deactivate: function () { $j(".UniGraph").removeClass("Move") }
        });
        ...
        $j(".GraphMenu." + this.droppableScope + " .BigButton").bind("drag", graphControlHandler.toolbarDragHandler);
