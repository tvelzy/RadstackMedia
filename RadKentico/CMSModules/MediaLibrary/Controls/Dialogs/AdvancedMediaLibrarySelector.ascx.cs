    ...
    private string GetSitesWhere()
    {
        ...
        switch (Sites)
        {
            case AvailableSitesEnum.OnlySingleSite:
                ...
                {
                    where = SqlHelperClass.AddWhereCondition(where, String.Format("SiteName LIKE N'{0}'", SqlHelperClass.GetSafeQueryString(SelectedSiteName)));
                }
                break;

            case AvailableSitesEnum.OnlyCurrentSite:
                where = SqlHelperClass.AddWhereCondition(where, String.Format("SiteName LIKE N'{0}'", SqlHelperClass.GetSafeQueryString(CMSContext.CurrentSiteName)));
                break;
